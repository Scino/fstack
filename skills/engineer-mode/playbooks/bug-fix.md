...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured bug-fix model (default in engineer-mode's Models section) with a specific scope; review the diff.
...
5. Stage the commits so the failing repro lands before the fix in git history; the diff tells the story. See the **tdd** skill for the failing-test-first cadence when the bug has a cheap local test path; skip it when the test would be expensive, integration-heavy, or unclear.
   This is the canonical **sequence-verifiable-units** principle skill, the failing test first and the fix on top. Tests you keep must observe behavior per **principle-test-behavior-not-implementation**.
6. If two or more shipped fixes that share one premise fail the same gate, stop and apply **principle-attack-the-premise** before the next fix. Write the premise, take a census, then question the assumption.
7. Run **Opening a PR**.
